...
class Averaged_DQN(BaseAgent):
    ...
    def learn(self):
        ...
        with torch.no_grad():
            ...
            averaged_next_q = self.targets[0](next_state)
            for i in range(1 , self.num_targets):
                averaged_next_q = averaged_next_q + self.targets[i](next_state)

            averaged_next_q = averaged_next_q * (1 / self.num_targets)
            target_q = (
                reward + (1 - done) * self.gamma * averaged_next_q.max(1, keepdims=True).values
            )
    ...
    def update_target(self):
        #self.target_network.load_state_dict(self.network.state_dict())
        for i in reversed(range(1, self.num_targets)):
            self.targets[i].load_state_dict(self.targets[i-1].state_dict())
        self.targets[0].load_state_dict(self.network.state_dict())
